...
public class CampTutorialController2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (!PlayerPrefs.HasKey("CampOverviewTutorialCompleted") || PlayerPrefs.GetInt("TutorialCompleted") == 0)
        {
            StartCoroutine(ActivatePanel(overviewPanel, 0.8f));
        }
            
    }

    IEnumerator ActivatePanel(GameObject panel, float time = 0f) {
